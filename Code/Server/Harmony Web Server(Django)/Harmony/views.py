...
def showfile2(request):
    ...
    if len(boardList) == 0:
        ...
        for i in range(len(boardList)):
            exist_pn.append(boardList[i]['postnum'])
        if postnum in exist_pn:
            postnum+=1
    ...
    if request.method == "POST":
        ...
        form = FileForm(filename, filepath)
        if form.is_valid():
            form.save()
           ...
           if boardRow['postnum'] == postnum:
               return render(request, 'Harmony/readpost.html',
                             {'postnum': boardRow['postnum'], 'ID': boardRow['ID'], 'title': boardRow['title'],
                              ...
                              'filepath': boardRow['filepath'], 'filename': boardRow['filename'], })
